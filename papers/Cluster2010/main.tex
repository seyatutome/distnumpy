...
DistNumPy introduces a performance overhead compared to a lower-level programming language such as C/C++ or FORTRAN. To investigate this overhead we have implemented the Monte Carlo and the Jacobi benchmark in  C. The implementations use the same algorithm and they are optimized for sequential execution. 

Executions on the both architectures show that DistNumPy is roughly 50\% slower than the C implementation when executing the Jacobi method, but almost 10 times slower when executing the Monte Carlo simulation. The problem with the Monte Carlo simulation is the very large vector allocations, which are required by the vectorization of the ufunc used in the DistNumPy implementation.
