...
%This is due the change from a single CPU-core per node to multiple CPU-cores per node. At sixteen CPU-cores half of the communication is performed through the use of shared memory, which means that the communication, just like the computation, is bound by the limited memory bandwidth.

%To get a better picture of the memory bandwidth limitation observed in the Jacobi execution we have profiled the execution (Fig. \ref{fig:JacobiProfiling}). As expected the result shows that the percentages used with communication increases when the number of CPU-cores increases. However, the profile different between eight CPU-cores and sixteen CPU-cores is minimal, which is due to the limited memory bandwidth. Since each node makes use of multiple CPU-cores when executing on more than eight CPU-cores the memory bandwidth limitation increases the computation time. Because of the decreasing total running time and the fact that the amount of communication is constant when all eight nodes are in use, the percentages used with communication increases at 32 and 64 CPU-cores.
...
\begin{figure*}[p]%
  \begin{center}%
    \subfloat[\label{fig:JacobiProfiling1}]{\includegraphics[width=250px]{gfx/JacobiProfiling}}
%    \qquad
    \subfloat[\label{fig:JacobiProfiling2}]{\includegraphics[width=250px]{gfx/JacobiProfiling2}}
