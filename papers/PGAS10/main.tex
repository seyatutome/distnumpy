...
To get a better picture of the memory bandwidth limitation observed in the Jacobi execution we have profiled the execution (Fig. \ref{fig:JacobiProfiling}). As expected the result shows that the percentages used with communication increases when the number of CPU-cores increases. However, the profile different between eight CPU-cores and sixteen CPU-cores is minimal, which is due to the limited memory bandwidth. Since each node makes use of multiple CPU-cores when more than eight CPU-cores are used the memory bandwidth limitation increases the computation time. Still, because of the decreasing total running time and the fact that the communication time is constant when all eight nodes are in use, the percentages used with communication increases at 32 and 64 CPU-cores.
