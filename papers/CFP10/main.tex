...
We perform three experiments of sequential Python programs running on an Ethernet based cluster of SMP-nodes with a total of 64 CPU-cores. The results show an 88\% CPU utilization when running a Monte Carlo simulation, 63\% CPU utilization on an N-body simulation and a more modest 50\% on a Jacobi solver. The primary limitation in processor utilization is identified as SMP limitations and not the distributed aspects. Based on the experiments we find that it is possible to obtain significant speedup from using our new array-backend without changing the original Python code.
...
\section{NumPy}
Python has become a popular language for high performance computing even though the performance of Python programs is is much lower than that of compiled languages. The growing popularity is because Python is used as the coordinating language while the compute intensive tasks are implemented in a high performance language.
...
The primary NumPy interface is a Python interface and it is possible to use NumPy exclusively from Python. NumPy also have a C interface in which it is possible to access the same functionality as in the Python interface. The C interface makes it possible to access low level data structures like pointers to array data and thereby provides the possibility to implement arbitrary array operations efficiently in C. The two interfaces can be used interchangeably through the Python program.
...
%first data entry in the dimension will be used for all calculations along that dimension. For example, the following statement will produce a new matrix $C$ in which the vector $A$ is added element-by-element to every row in the matrix $B$: \[C_{MxN} = A_N+B_{MxN}\]
...
\item \textbf{Array-view} is a view of one array-base. The view can represent the whole array or only a sub-part of the array. It contains its own meta-data that describe which part of the array-base is visible and it can even add non-existing 1-length dimensions to the array-base. The array-view is manipulated directly by the user and from the users perspective the array-view is the array.
\end{itemize}
Array-views are not allowed to refer to each other, which means that the hierarchy is very flat with only three levels: main memory, array-base and array-view. However, multiple array-views are allowed to refer to the same array-base. This hierarchy is illustrated in figure \ref{fig:views}. 
...
\begin{figure*}%
  \begin{center}%
    \subfloat[\label{fig:MonteCarloStrong}]{\includegraphics[width=170px]{gfx/MonteCarloStrong}}
%    \qquad
    \subfloat[\label{fig:MonteCarloStrongZoomed}]{\includegraphics[width=170px]{gfx/MonteCarloStrongZoomed}}
    \caption{Speedup graph of the Monte Carlo simulation. Besides having different scale, the (b) graph only goes to 8 CPU-cores whereas (a) goes to 64 CPU-cores, the two graphs are identical.}%
...
\begin{figure*}%
  \begin{center}%
    \subfloat[\label{fig:JacobiStrong}]{\includegraphics[width=170px]{gfx/JacobiStrong}}
%    \qquad
    \subfloat[\label{fig:JacobiStrongZoomed}]{\includegraphics[width=170px]{gfx/JacobiStrongZoomed}}
    \caption{Speedup of the Jacobi solver. Besides having different scale, the (b) graph only goes to 8 CPU-cores whereas (a) goes to 64 CPU-cores, the two graphs are identical.}%
...
\begin{figure*}%
  \begin{center}%
    \subfloat[\label{fig:NbodyStrong}]{\includegraphics[width=170px]{gfx/NbodyStrong}}
%    \qquad
    \subfloat[\label{fig:NbodyStrongZoomed}]{\includegraphics[width=170px]{gfx/NbodyStrongZoomed}}
    \caption{Speedup of Newtonian N-body simulation. Besides having different scale, the (b) graph only goes to 8 CPU-cores whereas (a) goes to 64 CPU-cores, the two graphs are identical.}%
...
\subsection{Summary}
The benchmarks clearly show that DistNumPy has both good performance and scalability when execution is not bound by the memory bandwidth, which is evident from looking at the speedup from one to eight CPU-cores (one the right speedup graphs). As expected the scalability of the Monte Carlo simulation is better than the Jacobi and the N-body computation simply because of the reduced communication requirements. 
