...
The amount of time used by waiting on non-hidden communication depends on many factors -- some related to the runtime system and some related to the implementation. A general expression of the relationship is given by the following equation, which can be used to find the optimal batch-size and the optimal number of initial batch-size increasements.
...
\end{tabular}
\begin{eqnarray*}
WaitTime &=& l + \frac{b}{2^{n} B} + \sum_{i=1}^{n} \max \left(0, l+ \frac{s}{2^{i-1} B} - \frac{C s}{2^{i}}\right) \nonumber \\
&& +  \max \left(0, l+ \frac{b}{B} - C b\right)\left(\frac{t}{b} - 3 - 2^{n+1} \right)
\end{eqnarray*}
\end{footnotesize}
