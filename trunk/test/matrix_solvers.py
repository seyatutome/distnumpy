...
def jacobi(A, B, tol=0.005, init_val=0.0):
    ...
    print "jacobi solver - isdist:", dist
    h = zeros(shape(B), float, isdist=dist)
    dmax = 1.0
    n = 0
    ...
    while dmax > tol:
        ...
        print "TMP2: ", tmp2, " isdist:",tmp2.isdist()
        print "TMP3: ", tmp3, " isdist:",tmp3.isdist()
...
def gauss_seidel(A, B, tol=0.005, init_val=0.0):
    ...
    print "gauss_siedel - isdist:", dist
    if init_val == 0.0:
        h = zeros(shape(B), float, isdist=dist)
        hnew = zeros(shape(B), float, isdist=dist)
    else:
        h = init_val
        hnew[:] = h[:]
    dmax = 1.0
...
def sor(A, B, rf, tol=0.005, forcedIter=0):
    ...
    print "successive over-relaxation (sor) - isdist:", A.isdist(), ""
    h = zeros(shape(B), float, isdist=A.isdist())
    hnew = zeros(shape(B), float, isdist=A.isdist())
    dmax = 1.0
    n = 0
...
#A = array([[4, -1, -1, 0], [-1, 4, 0, -1], [-1, 0, 4, -1], [0, -1, -1, 4]], float, isdist=d)

#A = array(random_list([size,size]), float, isdist=d)
A = zeros([size,size], dtype=float, isdist=d)
add(A,42,A)

#B = array(random_list([size]), float, isdist=d)
B = zeros([size], dtype=float, isdist=d)
add(B,42,B)
